...
function ScheduleTest() {
    ...
    const handleSubmit = (values) => {
        ...
        const startDate = moment(values.scheduleDate).startOf("week");
        const endDate = moment(values.scheduleDate).endOf("week");

        const daysInWeek = [];
        let currentDate = startDate;

        while (currentDate.isSameOrBefore(endDate, "day")) {
            daysInWeek.push(currentDate.format("YYYY-MM-DD"));
        ...
        daysInWeek.forEach((day) => {
            const data = {
                scheduleId: {
                    sheduleDateTime: day + "T" + values.scheduleTime,
